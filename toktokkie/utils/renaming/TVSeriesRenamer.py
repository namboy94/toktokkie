...
class TVSeriesRenamer(object):
    """
    Class that handles the renaming of episode files using thetvdb.com and a exchangeable
    naming schemes
    """
        ...
        for directory in directories:
            ...
            for season in seasons:
                ...
                if not season.lower().startswith("season"):
                    ...
                    self.__add_season_to_episodes__(season_path, season_number, show_name)

            # Add the special episodes to the Episode list
            self.__add_specials_to_episodes__(specials, show_name)

    def __add_season_to_episodes__(self, season_directory: str, season_number: int, show_name: str) -> None:
        """
    ...
    def request_confirmation(self) -> List[RenamerConfirmation]:
        """
        Wraps all episodes in the Episode list in RenamerConfirmation objects to enable a
        manual approval process for the renaming by the user
    ...
    def start_rename(self, noconfirm: bool = False) -> bool:
        ...
        Normally, this method requires that the selection has been manually approved by the user
        using request_confirmation() and confirm(). However, this may be bypassed by passing the
