...
from toktokkie.utils.iconizing.procedures.ProcedureManager import ProcedureManager, GenericProcedure
...
class UnitTests(unittest.TestCase):
    ...
    def test_searching(self):
        ...
        name = self.form.search_result_list.itemFromIndex(self.form.search_result_list.selectedIndexes()[0]).text(4)
    ...
    def test_setting_directory(self):
        self.form.directory_edit.setText(os.path.join("temp_testing", "Game of Thrones"))
        self.assertEqual(self.form.directory_edit.text(), os.path.join("temp_testing", "Game of Thrones"))
        self.assertEqual(self.form.show_name_edit.text(), "Game of Thrones")
        self.assertEqual(self.form.episode_spin_box.value(), 11)
    ...
    def test_adding_episode_to_queue(self):
        ...
        self.assertEqual(0, len(self.form.search_result_list.selectedIndexes()))
        self.assertEqual(1, self.form.download_queue.count())
        self.assertEqual(self.form.download_queue_list[0].get_filename(), "1_test.txt")

        self.form.download_queue.selectAll()
        episode_name = self.form.download_queue.itemFromIndex(self.form.download_queue.selectedIndexes()[0]).text()
    ...
    def test_moving_queue_items(self):
        self.form.download_queue_list = [XDCCPack(IrcServer("irc.namibsun.net"), "xdcc_servbot", 1),
