...
class BatchDownloadManagerTkGui(GenericTkGui, BatchDownloadManager):
    ...
    def lay_out(self):
        ...
        self.destination_label.grid(column=0, row=0)
        self.destination.grid(columnspan=2, column=2, row=0)
        self.destination_browser.grid(column=1, row=0)

        self.show_label = self.generate_label("Show Name")
        self.show = self.generate_text_entry("")
        self.show_label.grid(columnspan=2, column=0, row=1)
        self.show.grid(columnspan=2, column=2, row=1)

        self.season_label = self.generate_label("Season Number")
        self.season = self.generate_text_entry("")
        self.season_label.grid(columnspan=2, column=0, row=2)
        self.season.grid(columnspan=2, column=2, row=2)

        self.episode_label = self.generate_label("Starting Episode Number")
        self.episode = self.generate_text_entry("optional")
        self.episode_label.grid(columnspan=2, column=0, row=3)
        self.episode.grid(columnspan=2, column=2, row=3)

        self.search_label = self.generate_label("Search Term")
        self.search_field = self.generate_text_entry("", self.search_xdcc)
        self.search_label.grid(columnspan=2, column=0, row=4)
        self.search_field.grid(columnspan=2, column=2, row=4)
        ...
        self.search_engine_label.grid(columnspan=2, column=0, row=5)
        self.search_engine_combo_box.grid(columnspan=2, column=2, row=5)

        self.search_button = self.generate_simple_button("Start Search", self.search_xdcc)
        self.search_button.grid(columnspan=4, column=0, row=6)
        ...
        self.download_engine_label.grid(columnspan=2, column=0, row=7)
        self.download_engine_combo_box.grid(columnspan=2, column=2, row=7)

        self.options_label = self.generate_label("Options")
        self.rename_check = self.generate_check_box("Automatic Rename", True)
        self.options_label.grid(columnspan=2, column=0, row=8)
        self.rename_check.grid(columnspan=2, column=2, row=8)
        ...
        self.main_icon_label.grid(columnspan=2, column=0, row=9)
        self.secondary_icon_label.grid(columnspan=2, column=0, row=10)
        self.main_icon_location.grid(columnspan=2, column=2, row=9)
        self.secondary_icon_location.grid(columnspan=2, column=2, row=10)
        self.method_label.grid(columnspan=2, column=0, row=11)
        self.method_combo_box.grid(columnspan=2, column=2, row=11)

        self.download_button = self.generate_simple_button("Start Download", self.start_download)
        self.download_button.grid(columnspan=4, column=0, row=12)

        """
        self.search_results = self.generate_multi_list_box(
            {"types": [int, str, int, str, str], "titles": ["#", "Bot", "Pack", "Size", "Filename"]})
        self.search_results.grid(columnspan=2, column=0, row=1)

        self.directory_content = self.generate_multi_list_box({"types": [str], "titles": ["File Name"]})
        self.directory_content.grid(columnspan=2, column=0, row=1)
        """
